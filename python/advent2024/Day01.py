...
# To make sure they don't miss anything, The Historians split into two groups, each searching the office and trying
...
class Solution:
    ...
    def part1(self) -> int:
        ...
        return sum([abs(x[0] - x[1]) for x in zip(sorted_left, sorted_right)])
    
    def part2(self) -> int:
        similarity_score = 0
        counter_right = Counter(self.right_list)
        for x in self.left_list:
            similarity_score += counter_right[x] * x
