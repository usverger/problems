...
# Because you are currently only printing the correctly-ordered updates, you will need to find the middle page number
...
class Solution:
    ...
    def __init__(self, input: str):
        ...
        # parse the updates from the rest of the file
        self.updates = []
        for i in range(delim + 1, len(lines)):
            u = lines[i].split(',')
            self.updates.append([int(x) for x in u])

    def check_one_rule(self, rule, update):
        left,right = rule
    ...
    def check_all_rules(self, update):
        for rule in self.rules:
            if not self.check_one_rule(rule, update):
                return False
        return True
    
    def find_correct_rules(self):
        correct = []
    ...
    def part1(self):
        correct = self.find_correct_rules()
        return sum([update[len(update)//2] for update in correct])
    
    def find_incorrect_rules(self) -> List[List[int]]:
        incorrect = []
        for update in self.updates:
    ...
    def fix_one_update(self, update: List[int]) -> List[int]:
        u = update[:]
        
        while not self.check_all_rules(u):

            for rule in self.rules:
                left,right = rule
                if not left in u or not right in u: continue

                while u.index(left) > u.index(right):
                    # the left number is to the right of the right number, so move it left 
                    left_index = u.index(left)
                    temp = u[left_index - 1] # can never go below 0
                    u[left_index - 1] = left
                    u[left_index] = temp
    ...
    def part2(self):
        incorrect = self.find_incorrect_rules()
        fixed = [self.fix_one_update(update) for update in incorrect]
