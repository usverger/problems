...
class Solution:
    ...
    def find_part1(self) -> List[str]:
        x = re.findall('mul\(\d{1,3},\d{1,3}\)', self.input, flags=re.DOTALL)
        # print(x)
        return x
    
    def find_part2(self) -> List[str]:
        x = re.findall("mul\(\d{1,3},\d{1,3}\)|do\(\)|don't\(\)", self.input, flags=re.DOTALL)
        return x

    def part1(self) -> int:
        instructions = self.find_part1()
        sum = 0
    ...
    def part2(self) -> int:
        instructions = self.find_part2()
        #print(instructions)
        sum = 0
        do = True
        for i in instructions:
            if i.startswith('mul') and do:
                i = i.replace('mul(', '').replace(')', '')
                x, y = i.split(',')
                sum += int(x) * int(y)
            elif i == 'do()':
