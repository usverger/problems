...
class Solution:
    ...
    def findGroupBadges(self):
        ...
        for i in range(0,len(self.lines), 3):
            ...
            group = list(set(line1) & set(line2) & set(line3))[0]
            groups.append(group)
        return groups

    def findPart2(self):
        return sum([self.priority(x) for x in self.findGroupBadges()])

    def findCommonItem(self, line: List):
    ...
    def findPart1(self):
        sum = 0
        for l in self.lines:
            item = self.findCommonItem(l)
            sum += self.priority(item)
        return sum
    ...
    def priority(self, item: str) -> int:
        ...
        # A - 65
        # a - 97
        if item.isupper(): return ord(item) - 64 + 26
