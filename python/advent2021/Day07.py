...
# They seem to be preparing to blast a hole in the ocean floor; sensors indicate a massive underground cave system just beyond
...
class Solution:
    ...
    def __init__(self, input: str):
        self.crabs = list(map(int, input.split(',')))

    def align_crabs(self) -> int:
        positions = [0] * (max(self.crabs) + 1)
        for p in range(len(positions)):
            positions[p] = sum([abs(p - x) for x in self.crabs]) # cost of alignment to this position
        return min(positions)
    
    def align_crabs_part2(self) -> int:
        positions = [0] * (max(self.crabs) + 1)
        sums_cache = [0] * (max(self.crabs) + 1)
        
        for i in range(1, len(sums_cache)):
            sums_cache[i] = sums_cache[i-1] + i

        for p in range(len(positions)):
            positions[p] = sum([sums_cache[abs(p - x)] for x in self.crabs]) # cost of alignment to this position
