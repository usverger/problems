...
class Solution:
    ...
    def __init__(self, file: str):
        ...
        self.octopuses = [[int(i) for i in line] for line in self.lines]
    ...
    def step(self) -> int:
        ...
        for i in range(10):
            for j in range(10):
                ...
                # if the octopus has energy level > 9, flash
                queue = []
                ...
                while queue:
                    ...
                    flashes += self.flash_one(queue, candidate[0], candidate[1])
        return flashes
    
    def flash_one(self, queue: List[tuple], i: int, j: int) -> int:
        if i < 0 or i >= 10 or j < 0 or j >= 10: return 0 # check boundaries

        if self.octopuses[i][j] <= 0: return 0 # if the octopus has just flashed, we don't want to flash again
    ...
    def part2(self) -> int:
        step = 0
        while not all(self.octopuses[i][j] == 0 for i in range(10) for j in range(10)):
            self.step()
            step += 1
        return step
