...
# the safe thing to do is to figure out which board will win last and choose that one. That way, no matter which boards it picks,
...
class Solution:
    ...
    def __init__(self, lines: List[str]):
        self.numbers, self.boards = self.parse(lines)

    def parse(self, lines: List[str]):
        draws = [int(x) for x in lines[0].split(',') if x]
        boards = []
    ...
    def is_winner(self, board: List[List[int]], marked: set) -> bool:
        # check rows
        for row in board:
            if all(number in marked for number in row):
                return True
            
        # check columns
        for col in range(5):
            if all(board[row][col] in marked for row in range(5)):
                return True
            
        return False
    
    def umarked_sum(self, board: List[List[int]], marked: set) -> int:
        s = 0
    ...
    def part1(self) -> int:
        ...
        for number in self.numbers:
            ...
            for board in self.boards:
                if self.is_winner(board, marked):            
                    score = self.umarked_sum(board, marked) * number
                    return score
