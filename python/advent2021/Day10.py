...
# was found once (1197 points), and an illegal > was found once (25137 points). So, the total syntax error score for this file is
...
# You can only use closing characters (), ], }, or >), and you must add them in the correct order so that only legal pairs are formed and
# all chunks end up closed.
...
# (There will always be an odd number of scores to consider.) In this example, the middle score is 288957 because there are the same number of
...
class Solution:
    ...
    def check_one(self, line: str) -> int:
        stack = []
        for c in list(line):
            if c in '([{<':
                stack.append(c)
    ...
    def match(self, open: str, close: str) -> bool:
        return open == '(' and close == ')' \
            or open == '[' and close == ']' \
            or open == '{' and close == '}' \
            or open == '<' and close == '>'
    
    def score_part1(self, c: str) -> int:
        if c == ')':
            return 3
        elif c == ']':
            return 57
    ...
    def part1(self) -> int:
        ...
        for line in self.lines:
            score += self.check_one(line)   
        return score

    def check_one_return_stack(self, line: str) -> List[str]:
        stack = []
        for c in list(line):
            if c in '([{<':
                stack.append(c)
    ...
    def part2(self) -> int:
        ...
        for line in self.lines:
            stack = self.check_one_return_stack(line)
            if not stack: continue

            score = 0
            while stack:
                v = stack.pop()
                score = score * 5 + self.score_part2(v)
            scores.append(score)
    ...
    def score_part2(self, c: str) -> int:
        if c == '(':
            return 1
        elif c == '[':
            return 2
