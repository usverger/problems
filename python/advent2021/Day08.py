...
# but those wires are connected to segments randomly. Worse, the wire/segment connections are mixed up separately
# for each four-digit display! (All of the digits within a display use the same connections, though.)
...
# fgae cfgab fg bagce
# Because the digits 1, 4, 7, and 8 each use a unique number of segments, you should be able to tell which combinations of signals correspond
# to those digits. Counting only digits in the output values (the part after | on each line),
...
class Solution:
    ...
    def __init__(self, file: str):
        ...
        for i in range(0, len(lines)):
            ...
            self.signals.append([sorted(x) for x in input.split() + output.split()])
            self.outputs.append([sorted(x) for x in output.split()])
    ...
    def part1(self) -> int:
        ...
        # 6 -> 6 segments
        # 7 -> 3 segments (acf)
        # 8 -> 7 segments (abcedfg)
        # 9 -> 6 segments
        c = Counter([len(element) for one_output in self.outputs for element in one_output])

        return c[2] + c[4] + c[3] + c[7]
    
    def part2_bruteforce(self) -> int:
        CANONICAL = {
            "abcefg": 0,
            "cf": 1,    
            "acdeg": 2,
        ...
        for signal_index, signal_sample in enumerate(self.signals):

            for perm in permutations("abcdefg"): # try all possible permutations of segments
                mapping = dict(zip("abcdefg", perm))
                
                # try translating all signals in signal sample. All must match to canonical values
                valid = True
                for signal in signal_sample:
                    translated = self.translate(mapping, signal) 
                    if not translated in CANONICAL:
                        valid = False
                        break # the permutation is not value, try next one

                if not valid:
                    continue # this permutation is not valid, try next one

                # the permutation can be translated, save it and take next signal sample
                result = []
                for output in self.outputs[signal_index]:
                    translated = self.translate(mapping, output)
                    translated_digit = CANONICAL[translated]
                    result.append(translated_digit)

                numeric_result = int("".join(map(str, result)))
                results.append(numeric_result)
                break

        return sum(results)

    def translate(self, map, signal):
        return "".join(sorted([map[s] for s in signal]))
